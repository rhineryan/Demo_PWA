/* Summing.h  -- source for interface functions to the kernels
 defined in Summing.cl, the corresponding binaries are in binfiles/Summing.bin
...
namespace Summing_GPU{
	 ...
	 int prepare_kernels(const DeviceInterface * dev){
		 FILE * input = fopen("binfiles/Summing.bin","rb");
		 if(!input){
			 	std::cerr << "Loading binary file binfiles/Summing.bin failed \n";
			 	return FAILURE;
		 }
