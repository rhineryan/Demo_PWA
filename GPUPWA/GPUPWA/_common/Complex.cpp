/* Complex.h  -- source for interface functions to the kernels
...
namespace Complex_GPU{
	 ...
	 int prepare_kernels(const DeviceInterface * dev){
		 FILE * input = fopen("binfiles/Complex.bin","rb");
		 if(!input){
			 	std::cerr << "Loading binary file binfiles/Complex.bin failed \n";
			 	return FAILURE;
		 }
