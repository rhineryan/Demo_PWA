...
void Inter_Contri_binbybin_in_output_v2(){
   ...
   {
   int debug = 0;
  TString Dis_name_total[4] = {"mx","m23","m12","m13"};
  void Cal_inter_all(TString Dis_name,TString root_file,int N_Ac,int N_size2,int Npar[],int style,int color,bool leg_s,TString name_leg,int Nleg);
   TString wave_name ="/besfs/groups/psip/psipgroup/user/xiongxa/GamEtaPi0Pi0/Alg_gg_xl/GPUPWA/gpupwa2.1_1/Sampling_v2/0mp/plot_dir/input/wave.inp";
  TString root_file ="/besfs/groups/psip/psipgroup/user/xiongxa/GamEtaPi0Pi0/Alg_gg_xl/GPUPWA/gpupwa2.1_1/Sampling_v2/0mp/plot_dir/plot.root";
...
//**********Read waves we wanted ***********************************************************
  ...
  TH1F* H_data; 
  TString data_name = Dis_name + "_Data";
  H_data= (TH1F*)datafile1->Get(data_name);
  ...
  //Cal_inter_all(Dis_name,root_file,N_Ac,N_Ac,Npar_total,21,2,leg_s,"Data",Nleg); Nleg++;
  ...
  for(int j=0;j<Want_nwave;j++)
  {
    ...
    //if(j+3<10) {Cal_inter_all(Dis_name,root_file,N_Ac,Want_waveid[j][99],Npar_tem,24+kk,j+3,leg_s,Reso_want[j],Nleg); kk++;Nleg++;}
    if(j+3<10) {Cal_inter_all(Dis_name,root_file,N_Ac,Want_waveid[j][99],Npar_tem,25,j+3,leg_s,Reso_want[j],Nleg); kk++;Nleg++;}
    //if(j+3>9) {Cal_inter_all(Dis_name,root_file,N_Ac,Want_waveid[j][99],Npar_tem,24+kk,(j+3)-8,leg_s,Reso_want[j],Nleg); kk++;Nleg++;}
    if(j+3>9) {Cal_inter_all(Dis_name,root_file,N_Ac,Want_waveid[j][99],Npar_tem,25,(j+3)-8,leg_s,Reso_want[j],Nleg); kk++;Nleg++;}
  }
...
//##################################xiongxa##########################################//
  ...
  TString Root_file = "/besfs/groups/psip/psipgroup/user/xiongxa/GamEtaPi0Pi0/Alg_gg_xl/GPUPWA/gpupwa2.1_1/Bin_Bin_0mp/Version_2/" + tem + "/plot_dir/plot.root";
  TString wave_name = "/besfs/groups/psip/psipgroup/user/xiongxa/GamEtaPi0Pi0/Alg_gg_xl/GPUPWA/gpupwa2.1_1/Bin_Bin_0mp/Version_2/" + tem + "/plot_dir/input/wave.inp";

  TString his_data = File_name + "data" + tem;
  TFile* datafile1 =new TFile(Root_file);
  //TH1F* H_data = new TH1F(his_data,his_data,Nbin,low,up); 
  TH1F* H_data = new TH1F(his_data,his_data,80,2,2.8); 
  TH1F* H_data0;
...
//**********Read waves we wanted ***********************************************************
  ...
  for(int idraw=0;idraw<N_want;idraw++)
  {
    ...
    if(idraw==0) {TGraph *gr0 = new TGraph (nbins, fit_mx, fit_item_tem);gr0->SetMarkerColor(3);gr0->Draw("P*");} 
    if(idraw==1) {TGraph *gr1 = new TGraph (nbins, fit_mx, fit_item_tem);gr1->SetMarkerColor(4);gr1->Draw("P*");} 
    if(idraw==2) {TGraph *gr2 = new TGraph (nbins, fit_mx, fit_item_tem);gr2->SetMarkerColor(12);gr2->Draw("P*");} 
    if(idraw==3) {TGraph *gr3 = new TGraph (nbins, fit_mx, fit_item_tem);gr3->SetMarkerColor(6);gr3->Draw("P*");} 
    if(idraw==4) {TGraph *gr4 = new TGraph (nbins, fit_mx, fit_item_tem);gr4->SetMarkerColor(7);gr4->Draw("P*");} 
    if(idraw==5) {TGraph *gr5 = new TGraph (nbins, fit_mx, fit_item_tem);gr5->SetMarkerColor(8);gr5->Draw("P*");} 
    if(idraw==6) {TGraph *gr6 = new TGraph (nbins, fit_mx, fit_item_tem);gr6->SetMarkerColor(9);gr6->Draw("P*");} 
    if(idraw==7) {TGraph *gr7 = new TGraph (nbins, fit_mx, fit_item_tem);gr7->SetMarkerColor(10);gr7->Draw("P*");} 
    if(idraw==8) {TGraph *gr8 = new TGraph (nbins, fit_mx, fit_item_tem);gr8->SetMarkerColor(11);gr8->Draw("P*");} 
    if(idraw==9) {TGraph *gr9 = new TGraph (nbins, fit_mx, fit_item_tem);gr9->SetMarkerColor(12);gr9->Draw("P*");} 

  }
   gr0->Draw("P*");
   gr0->GetYaxis()->SetRangeUser(0,3500);
   gr0->GetYaxis()->SetTitle("Events/(0.05GeV/c^{2})");
   gr0->GetXaxis()->SetTitle("M(#eta#pi^{0}#pi^{0})(GeV/c^{2})");
   gr0->GetYaxis()->CenterTitle();
   gr0->GetXaxis()->CenterTitle();

   TLegend *leg = new TLegend(.783,.2,.938,.5,NULL,"brNDC");
   leg->SetTextSize(0.04237288);
   leg->SetTextFont(42);
   leg->SetLineColor(0);
   leg->SetLineStyle(1);
   ...
   for(int idraw=0;idraw<N_want;idraw++)
   {
     ...
     TString leg_name = Reso_want_tem +" output";
...
void plotgraph(TH1F* H1,const int Num,int MarkerStyle,int MarkerColor,TString name_leg,int Nleg)
{
  ...
  for(int i=0;i<Num;i++)
  {
    double Nsum = H1->Integral(1+5*i,5+5*i);
    fit_mx[i] = 2.0+0.05*i+0.025;
    fit_dat[i] = Nsum/10.0; //Scale the input to output
    sum_tem+=fit_dat[i];
    n++;
  }
