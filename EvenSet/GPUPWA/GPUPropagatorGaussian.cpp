#include "GPUPropagatorGaussian.h"
#ifdef USECPU
...
GPUPropagatorGaussian::GPUPropagatorGaussian(char * name,GPUStreamScalar & _mx2):
...
GPUPropagatorGaussian::~GPUPropagatorGaussian(void)
{
}


GPUDataStream<float2> * GPUPropagatorGaussian::operator()(int index, int block){
	...
	kernelgaussian(mList->GetDeviceInterface(), mx2stream,mass, mass*mass, width, mstream[index][block]);
...
GPUDataStream<float2> * GPUPropagatorGaussian::GetMassDerivative(int index, int block){
	assert(index < mx2.GetNStreams());
...
GPUDataStream<float2> * GPUPropagatorGaussian::GetWidthDerivative(int index, int block){
	assert(index < mx2.GetNStreams());
...
GPUDataStream<float> * GPUPropagatorGaussian::GetAbsMassDerivative(int index, int block){
	assert(index < mx2.GetNStreams());
...
GPUDataStream<float> * GPUPropagatorGaussian::GetAbsWidthDerivative(int index, int block){
	assert(index < mx2.GetNStreams());
