/* Tensors_cpu.h  -- source for interface functions to the kernels
 defined in /scratchfs/bes/xiongxa/Demo_PWA_GPUPWA/GPUPWA/GPUPWA/Tensors.cl, the corresponding binaries are in binfiles/Tensors_cpu.bin
...
namespace Tensors_CPU{
	 ...
	 int prepare_kernels(const DeviceInterface * dev){
		 FILE * input = fopen("binfiles/Tensors_cpu.bin","rb");
		 if(!input){
			 	std::cerr << "Loading binary file binfiles/Tensors_cpu.bin failed \n";
			 	return FAILURE;
		 }
