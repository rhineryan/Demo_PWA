/* Contractions_cpu.h  -- source for interface functions to the kernels
 defined in /scratchfs/bes/xiongxa/GamEtapKsKs/GPUPWA/gpupwa2.1_jpc2370check/GPUPWA/Contractions.cl, the corresponding binaries are in binfiles/Contractions_cpu.bin
...
namespace Contractions_CPU{
	 ...
	 int prepare_kernels(const DeviceInterface * dev){
		 FILE * input = fopen("binfiles/Contractions_cpu.bin","rb");
		 if(!input){
			 	std::cerr << "Loading binary file binfiles/Contractions_cpu.bin failed \n";
			 	return FAILURE;
		 }
