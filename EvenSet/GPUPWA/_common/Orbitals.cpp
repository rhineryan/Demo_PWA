/* Orbitals.h  -- source for interface functions to the kernels
 defined in /scratchfs/bes/xiongxa/Demo_PWA_GPUPWA/GPUPWA/GPUPWA/Orbitals.cl, the corresponding binaries are in binfiles/Orbitals.bin
...
namespace Orbitals_GPU{
	 ...
	 int prepare_kernels(const DeviceInterface * dev){
		 FILE * input = fopen("binfiles/Orbitals.bin","rb");
		 if(!input){
			 	std::cerr << "Loading binary file binfiles/Orbitals.bin failed \n";
			 	return FAILURE;
		 }
