...
mcjobprefix='A' ### just for convenience to change
fitjobprefix='B'
checkjobprefix='C'
...
do
  ...
  do   ###### DeadRun1111111 start
	...
	echo ${wave_ini[$i]} >> input/wave.inp
      ...
      ~xulei/mypwa/ConvConfig_xxa2xl.pl  input/wave.inp  input/fileconfig.txt
      ~xulei/mypwa/GenParIniRan_unfac.py input/fileconfig.txt  input/para.inp
      ~xulei/mypwa/GenJob2par.sh  ${checkjobprefix}_M${checkruns}_${check_n}.sh  MCInte_GEtaPi0Pi0
      gpu2q   ${checkjobprefix}_M${checkruns}_${check_n}.sh
      checkmctimes=0
      while true
      do
	jobflag=`grep 'End===End'  MCInte_GEtaPi0Pi0.log`
	...
	cd ..
	cp -s  ../GammaEtaPi0Pi0Analysis_Amplitude_MC_Integral    .
	~xulei/mypwa/GenJob2par.sh  ${checkjobprefix}_${checkruns}_${check_n}_$k.sh  Fit_GEtaPi0Pi0
	gpu2q  ${checkjobprefix}_${checkruns}_${check_n}_$k.sh
	cd ..
      done
      ~xulei/mypwa/checkgpujob.sh   ${checkjobprefix}_${checkruns}_${check_n}_

      like_1_test=`cat Fit_*/multifitresults_GammaEtaPi0Pi0Analysis_*.txt |awk '{print $6}'|sed 's#:##'|sed '/^$/d'|sort -nr|tail -n1`
      like_1=`cat Fit_*/multifitresults_GammaEtaPi0Pi0Analysis_*.txt |awk '{print $6}'|sed 's#:##'|sed '/^$/d'|sort -nr|~xulei/mypwa/check_conv_simple2.pl`
